...
Needs the Go that pixi.lock pins, and not merely some Go. Two releases of Go
produce two different indexes, both correct, and a diff check cannot tell that
apart from a mistake. So this reads the pin out of the lock file and skips
loudly on anything else, which is what makes `pixi run check` safe to run on a
machine with its own Go, such as a GitHub runner or a contributor's laptop.
Without Go at all it skips the same way, so working offline is fine.

CI still checks the index because the oracle environment installs the pinned Go
and runs `generated-check` against it. Moving to a newer Go is a lock file
update followed by `pixi run gen`, and the diff is the review.
"""
...
# `https://conda.anaconda.org/conda-forge/linux-64/go-1.26.7-he1b14cc_0.conda`
...
def pinned() -> set[str]:
    """The Go releases pixi.lock installs, spelled the way `go version` is.

    One entry per platform and all three are the same release, so this is
    normally a set of one. It is a set rather than a string because a lock file
    that pinned two would be worth failing on rather than picking from.
    """
    if not LOCK.is_file():
        return set()
    matches = (PIN.search(line.strip()) for line in LOCK.read_text().splitlines())
    return {f"go{match.group(1)}" for match in matches if match}
...
def generate() -> dict[str, str]:
    ...
    if want and have not in want:
        # Somebody else's Go, on a runner or a laptop. It would generate an
        # index that is right for that Go and wrong for this repository, and
        # the diff check would call that a mistake.
        print(
            f"gen: pixi.lock pins {' and '.join(sorted(want))} and the go on PATH is "
            f"{have}, so the Go API index is left as it is"
        )
        return {}
    ...
    header = [
        ...
        "# Generated by tools/gen/goapi.py. Do not edit by hand, run `pixi run -e oracle gen`.",
        "#",
