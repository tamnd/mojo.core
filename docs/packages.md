...
| `math/big` | 166 | `core.math.big` | Port | `Int`, `Rat`, `Float`. Arbitrary precision. `Int` and the limb layer under it are done, with `Format` and `Scan` waived because they are Go's `fmt.Formatter` and `fmt.Scanner` and there is no run time verb here to hand them. Karatsuba for the wide multiplies, a divisor table for base conversion, Montgomery and a sliding window for the modular exponent, Baillie-PSW for the primality test. The destination argument every Go method takes is a return value instead, and a panic is a raise. `Rat` and `Float` are still owed. [deviations](deviations.md). |
