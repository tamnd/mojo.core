...
Counts are exported symbols taken from Go's own `api/go1.*.txt` manifests, deduplicated across platform variants. They are the target, not an estimate of effort: `unicode`'s 328 symbols are mostly generated range tables and are a week's work, while `regexp`'s 50 are a matcher and are not.
...
Totals: **135 implemented** (11,598 symbols, plus generated `syscall` bindings), **41 excluded** (3,263 symbols).
...
| `io` | 107 | `core.io` | Adapt | `Reader`/`Writer` become a trait plus an erased vtable. The single most important design here. [design](design.md). |
| `io/fs` | 105 | `core.io.fs` | Adapt | Same treatment; `fs.FS` is an interface. |
| `bufio` | 80 | `core.bufio` | Port | Composes over `core.io`, which is the proof that the erasure design works. |
...
| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `math` | 136 | `core.math` | Wrap | `std.math` covers most of it. `core.math` adds the missing IEEE-754 edges and Go's exact naming. |
| `math/bits` | 53 | `core.math.bits` | Wrap | Over `std.bit`. |
| `math/cmplx` | 27 | `core.math.cmplx` | Wrap | Over `std.complex`. |
| `math/big` | 181 | `core.math.big` | Port | `Int`, `Rat`, `Float`. Arbitrary precision, and the one place in `core` where Mojo's SIMD is worth reaching for on the limb loops. |
...
| `math/rand` | 47 | none | Excluded | Superseded by v2 in Go itself. |
| `time` | 189 | `core.time` | Port | `Time`, `Duration`, `Month`, `Location`, monotonic readings, the whole format-layout language. [design](design.md). |
...
| `regexp` | 50 | `core.regexp` | Port | RE2 semantics: linear time, no backtracking, no catastrophic blowup. |
| `regexp/syntax` | 178 | `core.regexp.syntax` | Port | Parser, simplifier, compiler. Parse tree in an arena. |
| `text/scanner` | 59 | `core.text.scanner` | Port | |
| `text/tabwriter` | 19 | `core.text.tabwriter` | Port | |
...
| `text/template/parse` | 262 | `core.text.template.parse` | Port | Arena again. |
| `html` | 2 | `core.html` | Port | |
| `html/template` | 76 | `core.html.template` | Adapt | The contextual auto-escaper, over the adapted `text/template`. |
| `index/suffixarray` | 7 | `core.index.suffixarray` | Port | |
| `mime` | 16 | `core.mime` | Port | |
...
| `encoding/json` | 71 | `core.encoding.json` | Adapt | Two paths: a dynamic arena document, and generated codecs for known struct shapes. [design](design.md). |
| `encoding/xml` | 83 | `core.encoding.xml` | Adapt | Same two paths. |
...
| `encoding/binary` | 36 | `core.encoding.binary` | Port | Byte orders, varints, and a `Read`/`Write` that is generated rather than reflected. |
| `encoding/csv` | 34 | `core.encoding.csv` | Port | Fallible iteration, so `Reader.records()` is `has_next`/`next`. |
| `encoding/base64` | 24 | `core.encoding.base64` | Wrap | `std.base64` exists but is smaller than Go's. Extended, not forked. |
| `encoding/base32` | 21 | `core.encoding.base32` | Port | |
| `encoding/hex` | 15 | `core.encoding.hex` | Wrap | Over `std.base64`'s b16. |
| `encoding/ascii85` | 7 | `core.encoding.ascii85` | Port | |
| `encoding/asn1` | 73 | `core.encoding.asn1` | Adapt | DER. Reflection-driven in Go; generated here. Needed by `x509`. |
...
| `hash/adler32` | 4 | `core.hash.adler32` | Port | |
| `hash/crc32` | 16 | `core.hash.crc32` | Port | Slicing-by-8, plus the hardware instruction where the target has it. |
| `hash/crc64` | 11 | `core.hash.crc64` | Port | |
| `hash/fnv` | 6 | `core.hash.fnv` | Port | |
| `hash/maphash` | 18 | `core.hash.maphash` | Wrap | Over `std.hashlib`. |
| `crypto` | 67 | `core.crypto` | Adapt | The registry of hash functions, without reflection. |
| `crypto/subtle` | 8 | `core.crypto.subtle` | Port | Constant-time primitives. Lint-guarded: no branch on secret data. |
| `crypto/md5` `sha1` `sha256` `sha512` `sha3` | 72 | `core.crypto.*` | Port | Native Mojo, vectorized, checked against NIST vectors. |
| `crypto/hmac` `hkdf` `pbkdf2` | 6 | `core.crypto.*` | Port | |
| `crypto/aes` `des` `rc4` `cipher` | 55 | `core.crypto.*` | Port | AES via the AES-NI/NEON instructions with a constant-time software fallback. |
| `crypto/rand` | 5 | `core.crypto.rand` | Port | `getrandom` on Linux, `getentropy` on macOS. Never the userspace PRNG. |
| `crypto/ed25519` `ecdh` `ecdsa` `elliptic` `rsa` | 188 | `core.crypto.*` | Wrap | OpenSSL-backed, Go's API shape. `elliptic`'s deprecated custom-curve entry points are excluded. |
| `crypto/mlkem` `hpke` | 85 | `core.crypto.*` | Wrap | OpenSSL 3.5 and later. Degrades with a clear error when the linked OpenSSL is older. |
| `crypto/fips140` | 4 | `core.crypto.fips140` | Adapt | Reports the mode of the OpenSSL provider rather than of a Go module. |
| `crypto/tls` | 341 | `core.crypto.tls` | Wrap | OpenSSL. Presents a `core.io` stream, so everything above it is protocol-agnostic. |
| `crypto/x509` | 325 | `core.crypto.x509` | Wrap | OpenSSL for chain building and verification; Go's API shape over it. |
| `crypto/x509/pkix` | 51 | `core.crypto.x509.pkix` | Port | Pure data types over `core.encoding.asn1`. |
| `crypto/dsa` | 24 | none | Excluded | Deprecated in Go. |
...
| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `compress/flate` | 38 | `core.compress.flate` | Port | The base everything else here needs. |
| `compress/gzip` | 33 | `core.compress.gzip` | Port | |
| `compress/zlib` | 25 | `core.compress.zlib` | Port | |
| `compress/bzip2` | 3 | `core.compress.bzip2` | Port | Decompression only, as in Go. |
| `compress/lzw` | 15 | `core.compress.lzw` | Port | Needed by `image/gif`. |
| `archive/tar` | 85 | `core.archive.tar` | Port | Fallible iteration over entries. |
| `archive/zip` | 78 | `core.archive.zip` | Port | |
| `image` | 281 | `core.image` | Adapt | `image.Image` is an interface; trait plus vtable. Most of the 281 are concrete pixel-format types. |
| `image/color` | 78 | `core.image.color` | Port | |
| `image/color/palette` | 2 | `core.image.color.palette` | Port | Generated tables. |
| `image/draw` | 25 | `core.image.draw` | Port | |
| `image/png` | 24 | `core.image.png` | Port | |
| `image/jpeg` | 15 | `core.image.jpeg` | Port | |
| `image/gif` | 22 | `core.image.gif` | Port | |

## Operating system

| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `os` | 293 | `core.os` | Port | Files, directories, environment, process. Over `core.syscall`, not over `std.os`, because `std.os` is much smaller and its error model is not this one. |
...
| `path/filepath` | 34 | `core.path.filepath` | Port | Host paths. Two platforms, so `Separator` is always `/`; the abstraction is kept for portability rather than deleted. |
| `syscall` | (per-platform) | `core.syscall` | Adapt | Generated from the host headers for macOS and Linux, arm64 and x86-64, by `tools/baseline/`. Go's 17,083 figure is one platform's table times eleven; ours is two times two. |
| `runtime/debug` | 37 | `core.runtime.debug` | Adapt | Stack traces, build info, GC knobs that are no-ops with a documented reason. |
| `runtime/metrics` | 27 | `core.runtime.metrics` | Adapt | Reports the `core` scheduler's metrics, not Go's. |
...
| `debug/elf` | 3,296 | `core.debug.elf` | Port | Mostly generated constant tables. |
| `debug/macho` | 458 | `core.debug.macho` | Port | |
| `debug/dwarf` | 637 | `core.debug.dwarf` | Port | |
...
| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `net` | 381 | `core.net` | Adapt | TCP, UDP, Unix, IP. `net.Conn` and `net.Listener` are interfaces; trait plus vtable. Includes the resolver, with Happy Eyeballs. |
...
| `net/textproto` | 64 | `core.net.textproto` | Port | |
| `net/mail` | 20 | `core.net.mail` | Port | |
| `net/smtp` | 27 | `core.net.smtp` | Port | |
| `net/http` and 7 subpackages | 651 | none | Excluded | `mojo.httpx`. [design](design.md) covers the boundary and the migration. |
| `net/rpc`, `net/rpc/jsonrpc` | 62 | none | Excluded | Frozen in Go, and reflection to its foundations. |

## Databases

| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `database/sql` | 165 | `core.database.sql` | Adapt | The pool, transactions and statements port. `Rows.Scan(&a, &b)` is reflection over `any` destinations; here the destinations are a typed pack. [design](design.md). |
| `database/sql/driver` | 121 | `core.database.sql.driver` | Adapt | Nine interfaces, all erased vtables. `driver.Value` is a tagged union rather than `any`. |

## Logging, flags, diagnostics

| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `log` | 51 | `core.log` | Port | |
| `log/slog` | 172 | `core.log.slog` | Adapt | Structured logging. `slog.Any` needs reflection; here an attribute value is a closed union of the types a log line can hold. |
| `log/syslog` | 142 | `core.log.syslog` | Port | The count is platform variants; the real surface is about twenty. |
| `flag` | 93 | `core.flag` | Port | |
...
| Go package | Symbols | `core` package | Verdict | Note |
| --- | --- | --- | --- | --- |
| `testing` | 170 | `core.testing` | Wrap | Over `std.testing`. |
...
Forty-one packages, 3,263 symbols. Each line is a reason, not an apology.
...
`tools/parity/` reads Go's `api/go1.*.txt` manifests, applies the naming rules from [design](design.md), and compares the result against the symbols `mojo doc` reports for each `core` package. It prints what is missing, what is extra, and what is present under a name the rules did not predict.

It runs in CI on every commit and its output is the coverage percentage in the README. A package is not "done" because somebody says so; it is done when `tools/parity` says its row is at 100% and its tests pass.

The two escape hatches are explicit and both are files, not flags. `parity/waivers.toml` lists symbols deliberately not ported, one line each with a reason, and it is the machine-readable form of the Excluded section above. `parity/renames.toml` lists symbols whose Mojo name the rules could not derive. Both are reviewed like code, and a growing waivers file is the signal that the contract is being negotiated away rather than met.
