...
| `syscall` | (per-platform) | `core.syscall` | Adapt | Constants and layouts generated from the host headers for macOS arm64, Linux x86-64 and Linux arm64 by `tools/baseline/` and `tools/gen/syscall.py`, and the calls over descriptors, paths and `struct stat` written over them. `open` takes two arguments and refuses `O_CREAT`, because the three argument form is variadic and a variadic C function cannot be called portably from Mojo; `create` covers it. Go's manifests carry 6,233 names here across eleven platforms and ours is three, which are different enough that a percentage between them would mean nothing. Left out of the totals for that reason. |
