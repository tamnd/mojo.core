...
| `time` | 145 | `core.time` | Port | `Time`, `Duration`, `Month`, `Location`, monotonic readings, the whole format-layout language. [design](design.md). Instants, durations and the Gregorian calendar are in, and so is the whole zone half: `Location`, a `Time` that carries one and reads every field against it, `fixed_zone`, a reader for compiled zone files, and `load_location` and `local` for finding one in the host's database. The layout language goes both ways: `format`, `append_format` and the nineteen named layouts write an instant out, and `parse`, `parse_in_location` and `parse_duration` read one back, with every piece of the reference instant and every spelling of the zone on both sides. Not written yet are the timers and the marshalling methods. |
