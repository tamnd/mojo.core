...
| `time` | 145 | `core.time` | Port | `Time`, `Duration`, `Month`, `Location`, monotonic readings, the whole format-layout language. [design](design.md). Instants, durations and the Gregorian calendar are in, and every method reads UTC until `Location` lands. The layout language, the zone database and the timers are each their own piece of work and none of them is written yet. |
