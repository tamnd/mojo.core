...
| `io/fs` | 90 | `core.io.fs` | Adapt | Same treatment; `fs.FS` is an interface. Started with `valid_path`, the rule about names, which is lexical and needs none of the trait: a name in this package is slash separated, relative and already clean, whatever the host underneath spells its own paths like. `core.path.filepath.localize` is defined in terms of it. |
