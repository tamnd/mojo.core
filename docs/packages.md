...
| `time` | 145 | `core.time` | Port | `Time`, `Duration`, `Month`, `Location`, monotonic readings, the whole format-layout language. [design](design.md). Instants, durations and the Gregorian calendar are in, and so is `Location` itself, with `fixed_zone` and a reader for compiled zone files. A `Time` does not carry a location yet and there is no registry to look one up by name, so every method on `Time` still reads UTC. The layout language and the timers are each their own piece of work and neither is written yet. |
