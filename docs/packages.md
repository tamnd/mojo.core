...
| `time` | 145 | `core.time` | Port | `Time`, `Duration`, `Month`, `Location`, monotonic readings, the whole format-layout language. [design](design.md). Instants, durations and the Gregorian calendar are in, and so is `Location`, with `fixed_zone`, a reader for compiled zone files, and a `Time` that carries a location and reads every field against it. What is left of the zone half is `load_location` and `Local`, which is the part that has to find the host's database. The layout language and the timers are each their own piece of work and neither is written yet. |
