...
And `tools/probe/`, which is every claim in [design.md](design.md) as a compiled, running test. The thin function pointer vtable, `raises` across it, the field restriction on origins, the non-nullable pointer, the `for` loop swallowing an error out of `__next__`, comptime folding of a user `def`, the builtin only limit on `where` clauses, `@deprecated` firing per instantiation, and the pthread primitives.
