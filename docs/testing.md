...
`--shard 2/8` runs an eighth of the suite, and CI's race job is eight of those rather than one whole one. Instrumenting the whole library in one build costs far more than instrumenting it in eight: the tree took the better part of an hour, a shard of it takes one or two minutes, so the eight together are both faster and cheaper than the one was. The split is a contiguous run of the test files in sorted order rather than a round robin of them, because what a shard pays for is the packages its files reach and files that sort next to each other are in the same package or a neighbouring one; dealt round robin every shard would reach the whole library and the split would save nothing. It is by file and not by test for the same reason, since two shards holding halves of one file would build that file and everything under it twice. Every test is in exactly one shard, and `pixi run test-selftest` adds them back up against the tree rather than trusting that, because a shard that quietly drops a file reports a pass and so does the shard that should have had it.

`--deadline 900` stops the run after that many seconds, and CI passes it. A hang is the one failure that says nothing at all: the tests print when they fail and not otherwise, so a test waiting for something that never arrives leaves the job to be cancelled at its own limit an hour or two later with one line in the log about a limit and nothing about where it was. With a deadline the suite is sampled before it is stopped, so what lands in the log is the stack of every thread with the blocked one among them, which on a macOS runner names the Mojo function it was sitting in. A local run has no deadline, because the answer to a hang in front of you is a debugger.
...
A `# no-race: why` comment on the line above a test takes it out of the sanitised runs and out of no others. One test needs it, and the reason is the sanitiser's rather than ours. It wraps `read`, and a signal that arrives while a thread is inside one of its wrappers has its handler deferred until the wrapper returns; the wrapper here is around a read that `SA_RESTART` makes the kernel start again, so it never returns, the handler never runs, and the byte the test is waiting for is never written. The test is `tests/syscall/test_signal.mojo`'s one about a signal arriving while the program is blocked in a read, which is the property the signal pipe exists for, and it passes on every platform without the sanitiser. It cost two hours of a cancelled job to find, which is why the deadline above now exists.
