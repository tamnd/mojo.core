...
`errors.Join` is where this mechanism is genuinely weaker than Go's. Go holds several error values and all of their fields; at most one of the errors passed here still owns the thread's record, so the rest contribute their message and their place in the tree and nothing else. `errors.capture` is what closes that, and until an error is captured the gap is real and is listed in [deviations.md](deviations.md).
