...
`ArcPointer` is what stands in for a shared immutable table. Go's standard library hands out pointers into structures that are built once, never freed and read from everywhere, and `*time.Location` is the one this library met first: a `Time` holds one, copying a `Time` is free, and the garbage collector is what makes that safe. The answer here is an `ArcPointer` to an immutable value, which copies with an atomic increment rather than by copying the value, so a `Time` still copies cheaply and the location is freed when the last `Time` holding it goes. That is not free, an increment is not nothing, and it is still very much cheaper than the alternative, which was a process wide intern table keyed by name, living outside Mojo for the same reason the thread local slot does and leaking every location it was ever given. `probes/arc_pointer_shares.mojo` pins the two properties the choice rests on, that copies share one pointee and that the count survives four threads copying at once.

Narrowing a value and widening it straight back does not sign extend. `Int(Int32(raw))` on a `UInt32` whose bits are minus three hours gives 4294956496 rather than -10800, and so do a named intermediate and `Int32(raw).cast[DType.int64]()`: where the narrow value has no other use the narrowing is dropped and the original is zero extended. Two things make it worse than an ordinary bug. It is a wrong answer rather than a refusal, and a literal folds to the right one, so the mistake is invisible until the value comes out of a file. It cost this library a day in `core.time`'s TZif reader, where every zone west of Greenwich came out hours wrong and the tests missed it because the vendored files answered those instants from the cache instead. Anything reading a signed field out of a binary format subtracts the range by hand, which no optimiser can drop without changing the value. `probes/int_narrow_then_widen.mojo` pins all four spellings and will start failing when the first three are fixed.
...
A value cannot be moved out of a tuple either, and the refusal is a different one: "expression does not designate a value with an origin". That settles what a function returning two owned values can look like. `core.os.pipe` gives back `Tuple[File, File]` and its caller reads, writes and closes through `ends[0]` and `ends[1]` rather than naming the two ends, because a `File` cannot be copied and there is no way to take one out. Everything that borrows works, including handing an end to something that takes a `Reader` or a `Writer`; giving an end away to a value that owns it does not. `probes/move_out_of_tuple.mojo` pins it, and the day it compiles the signature already there becomes the one Go has.
